...
class XrDatasetMultiResTrainSupervised(XrDatasetSupervised):
    ...
    def __init__(self, multires=[1], satellite_vars=None, models_vars=None, models_paths=None,
        ...
        # below. Kept distinct from self.resize, which XrDatasetSupervised
        # .__init__ (called above) has already turned into the EFFECTIVE
        # raw-pixel coarsening factor for self.reference_source.
...
class XrDatasetMultiResTestSupervised:
    ...
    def __init__(self, multires=[1], patch_dims_dict=None, strides_test_dict=None,
        ...
        # Resolve which source drives the grid (each per-resolution
        # XrDatasetSupervised below will independently re-resolve the same
        # value) so that nominal multires levels (config units = asip's
        # 500m) can be converted to the raw pixel factor actually applicable
        # to that source. Validate every level up front so an unreachable
        # target fails fast here.
