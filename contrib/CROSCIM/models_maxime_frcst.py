...
class Lit4dVarNet_CROSCIM(Lit4dVarNet):
    ...
    def __init__(self,
            ...
            frcst=None,                 # NEW: forecast model for last resolution
        ...
        self.frcst = frcst  # Forecast model for last resolution (if any)
        ...
        self.frcst = self.frcst.to(device) 
        ...
        self.loss_target_ratios = {
            ...
            'grad': 0.3,      # 10% of total loss
            'prior': 0.0,     # 5% of total loss
            'tv': 0.0,#0.10,        # 10% of total loss
            'context': 0.,#0.05    # 5% of total loss
            'cimr_constraint': 0.0
        }
        
        # Running averages for auto-balancing (EMA with alpha=0.1)
        self.register_buffer('loss_ema', torch.zeros(6))  # [base, grad, prior, tv, context]
        self.ema_alpha = 0.1
        self.loss_names = ['base', 'grad', 'prior', 'tv', 'context','cimr_constraint']
    ...
    def configure_optimizers(self):
        if self.opt_fn is not None:
            ...
            params += list(filter(lambda p: p.requires_grad, self.frcst.parameters()))
    ...
    def crop_daw(self, item_dict, res, frcst_lead=0):
        last = self.len_daw[res] - frcst_lead
    ...
    def format_batch_for_solver(self, batch):
        ...
        for var in self.input_vars:
            if hasattr(batch, var):
                input_tensors.append(getattr(batch, var)[:, :-self.frcst_lead,:,:])  # On prend uniquement les premiers pas de temps pour l'input
            else:
        ...
        for var in self.tgt_vars:
            if hasattr(batch, var):
                #tgt_tensors.append(getattr(batch, var)[:, -self.frcst_lead:])  # On prend uniquement les derniers pas de temps pour le target
                tgt_tensors.append(getattr(batch, var)[:, :-self.frcst_lead,:,:])
            else:
            ...
            for batch_var in batch_dict:
                for alias in aliases:
                    if batch_var.lower().endswith(alias.lower()):
                        batch_dict[batch_var][:,:-self.frcst_lead,:,:] = batch_dict[batch_var][:,:-self.frcst_lead,:,:] - coarse_prediction
    ...
    def on_train_epoch_start(self):
        ...
        max_epochs_res = (3 * self.trainer.max_epochs) // 4 

        if epoch < max_epochs_res:

            n_res = len(self.multires)
            total_epochs = max_epochs_res
            ...
            for res in self.multires:
                ...
                if res == train_res:
                    ...
                    if self.trainer.is_global_zero:
                        ...
                        print(
                            f"[SOLVER] solver_x{res} | MODE=train | "
                    ...
                    if self.trainer.is_global_zero:
                        print(f"[SOLVER] solver_x{res} | MODE=eval | gradients frozen")

                # contrôle supplémentaire
                if self.trainer.is_global_zero:
                    grad_enabled = any(p.requires_grad for p in model.parameters())
                    print(
                        f"  ↳ check | training_flag={model.training} | "
                        f"grad_enabled={grad_enabled}"
                    )

            print("="*70 + "\n")
            self.frcst.eval()
            for p in self.frcst.parameters():
                p.requires_grad = False
        else:
            #mettre la dernière res en eval 
            model_last_res = self.solver.solvers[f"solver_x{self.multires[-1]}"]
            model_last_res.eval()
            for p in model_last_res.parameters():
                p.requires_grad = False
            #et le modele de forecast en train
            model = self.frcst
            model.train()
            for p in model.parameters():
                p.requires_grad = True
    ...
    def tensor_stats(self,name, x):
        mask = torch.isfinite(x)

        if mask.sum() > 0:
            xmin = x[mask].min().item()
            xmax = x[mask].max().item()
        else:
            xmin = float("nan")
            xmax = float("nan")
    ...
    def multistep(self, batch, phase=""):
        ...
        print("\n" + "-"*70)
        print(f"[MULTISTEP] phase={phase} | epoch={self.current_epoch}")
        print(f"multires={self.multires}")
        print("-"*70)
        ...
        if epoch < max_epochs_res:
            ...
            res_index = min(epoch // steps_per_res, n_res - 1)
            train_resolutions = [self.multires[res_index]]
            print(f"RECONSTRUCTION TASK - MULTIRESOLUTION TRAINING")
            print(f"[PROGRESSIVE] steps_per_res={steps_per_res}")
            print(f"[PROGRESSIVE] res_index={res_index}")
            print(f"[MULTISTEP] train_resolutions={train_resolutions}")
        else:
            train_resolutions = []
            should_forecast = True
            print(f"FORECASTING TASK")
        ...
        for i, res in enumerate(self.multires):
            ...
            model = self.solver.solvers[f"solver_x{res}"]
            print(
                f"model_mode={'train' if model.training else 'eval'} | "
                f"grad_enabled={any(p.requires_grad for p in model.parameters())}"
            )

            if hasattr(batch_res, "xc"):
                print(f"xc shape: {tuple(batch_res.xc.shape)}")
            if hasattr(batch_res, "yc"):
                print(f"yc shape: {tuple(batch_res.yc.shape)}")

            if i == 0:

                print("[COARSEST LEVEL]")

                if should_train:
                    print("[TRAIN] running step()")
                    loss, out[f"patch_x{res}"] = self.step(batch_res, res=res, phase=phase)
                    print(f"[LOSS x{res}] {loss.item():.6f}")
                    total_loss += loss
                    if loss > 0: 
                        print("\n===== LARGE LOSS DEBUG =====")

                        self.tensor_stats("target", getattr(batch_res, 'asip_sic'))
                        self.tensor_stats("pred", out[f"patch_x{res}"]['pred_sic'])

                        asip_sic = getattr(batch_res, 'asip_sic')[:,:-self.frcst_lead,:,:]
                        mask = torch.isfinite( asip_sic)
                        print("valid pixels:", mask.sum().item(), "/", mask.numel())

                        err = (out[f"patch_x{res}"]['pred_sic'] -  asip_sic)[mask]
                ...
                print(f"[FINE LEVEL] using coarse resolution x{coarser_res}")
                ...
                print(f"xc_target shape={tuple(xc_target.shape)}")
                print(f"xc_coarse shape={tuple(xc_coarse.shape)}")
                ...
                print("[GRAD FLOW] detach coarse outputs")
                out_coarse_for_interp = {
                    k: v.detach() if isinstance(v, torch.Tensor) else v
                    for k, v in out[f"patch_x{coarser_res}"].items()
                }
                ...
                out[f"patch_x{coarser_res}_on_x{res}"] = self.crop_daw(
                    out[f"patch_x{coarser_res}_on_x{res}"], res, frcst_lead=self.frcst_lead
                )



                print("[UPDATE BATCH] injecting anomaly")
                ...
                for var_name in self.tgt_vars:
                    ...
                    key = f"patch_x{coarser_res}_on_x{res}"
                    #print(f"OUT coarser res={out[key][pred_var_name].shape}")
                ...
                print("[RESIDUAL ADDITION] coarse prediction added")

            if res == self.multires[-1]:
                if should_forecast and phase == 'train':
                    print("[TRAIN forecast] running step()")
                    loss, out_frcst = self.step_frcst(out[f"patch_x{res}"], batch_res, phase=phase)
                    print(f"[LOSS x{res}] {loss.item():.6f}")
                    total_loss += loss
                else:
                    print("[FORWARD ONLY]")
                    with torch.no_grad():
                        _, out_frcst = self.step_frcst(out[f"patch_x{res}"], batch_res, phase=phase)
                ...
                for var_name in self.tgt_vars:
                    ...
                    out[f"patch_x{res}"][pred_var_name] = torch.cat([
                        out[f"patch_x{res}"][pred_var_name],
                        out_frcst[pred_var_name]
                                        ], dim=1)
            ...
            if phase == "test" and res == self.multires[-1]:
                if len(self.test_viz.get(res, [])) < self.max_test_viz:
                    self._log_test_example(
                        batch_res,
                        out[f"patch_x{res}"],
                        res
                    )
    ...
    def compute_balanced_weights(self, loss_values):
        ...
        return {
            ...
            'cimr_constraint': weights[5].item()
        }
    ...
    def step(self, batch, res, phase=""):
        ...
        total_cimr_constraint_loss = 0.0

        for var_name in self.tgt_vars:
            if not hasattr(batch, var_name):
                raise ValueError(f"Batch missing variable: {var_name}")
    
            target = getattr(batch, var_name)[:,:-self.frcst_lead,:,:]
            var_suffix = var_name.split('_', 1)[1]  # Get everything after first '_'
            pred_var_name = f'pred_{var_suffix}'
            pred = out[pred_var_name]
    
            # Masks
            mask_interp = ~batch._asdict()[self.var_mapping[var_name]][:,:-self.frcst_lead,:,:].isfinite() & target.isfinite()
            mask_obs = batch._asdict()[self.var_mapping[var_name]][:,:-self.frcst_lead,:,:].isfinite() & target.isfinite()
            ...
            mask_target = batch._asdict()[var_name][:,:-self.frcst_lead,:,:].isfinite()
            ...
            input_obs = batch._asdict()[self.var_mapping[var_name]][:,:-self.frcst_lead,:,:]
        ...
        if self.training:
            ...
            weights = {'base': 1.0, 'grad': 1.0, 'prior': 1.0, 'tv': 1.0, 'context': 1.0, 'cimr_constraint': 1.0}
    ...
    def step_frcst(self, reconstructed_field, batch, phase=""):
        ...
        for var_name in self.tgt_vars:
            ...
            if '_' in var_name:
                ...
                pred_var_name = f'pred_{var_name}'
            input_tensors.append(reconstructed_field[pred_var_name])  # Add the reconstructed variable as input for forecasting

        # Create target (forecast) field
        tgt_tensors = []
        for var in self.tgt_vars:
            if hasattr(batch, var):
                tgt_tensors.append(getattr(batch, var)[:, -self.frcst_lead:,:,:])
        ...
        print(f"INP RANGE: [{input_tensors[0].min().item():.4f}, {input_tensors[0].max().item():.4f}]")

        model_frcst = self.frcst
        out_frcst = model_frcst(sbatch) 
        out_frcst = self.split_tensor_to_dict(out_frcst)

        # for name, p in self.frcst.named_parameters():
        ...
        for i, var_name in enumerate(self.tgt_vars):
            ...
            target = getattr(batch, var_name)[:, -self.frcst_lead:,:,:]  # (B, T, Y, X)
            ...
            if '_' in var_name:
                ...
                pred_var_name = f'pred_{var_name}'
            pred = out_frcst[pred_var_name]  # (B, T, Y, X)
            ...
            loss = self.weighted_mse(
                ...
                self.optim_weight['frcst']
            )
            total_loss += loss
        return total_loss, out_frcst
    ...
    def base_step(self, batch, res, phase=""):
        ...
        for i, var_name in enumerate(self.tgt_vars):
            if not hasattr(batch, var_name):
                raise ValueError(f"Batch does not contain variable '{var_name}'")
            target = getattr(batch, var_name)[:, :-self.frcst_lead,:,:]  # (B, T-frcst_lead, Y, X)
            # Convert var_name to pred_var_name
            if '_' in var_name:
                var_suffix = var_name.split('_', 1)[1]
                pred_var_name = f'pred_{var_suffix}'
            ...
            mask = ~batch._asdict()[self.var_mapping[var_name]][:, :-self.frcst_lead,:,:].isfinite() & target.isfinite()
            # Mask 2: Observation pixels (both input and target valid)
            mask2 = batch._asdict()[self.var_mapping[var_name]][:, :-self.frcst_lead,:,:].isfinite() & target.isfinite()
    ...
    def convert_xr_to_batch(self, coarse, batch, spatial_sel=False):
        ...
        for var in self.tgt_vars + pred_vars + ["time", "yc", "xc"]:
            ...
            for i in range(nbatch):
                times_i = np.squeeze(times[i])[:-self.frcst_lead]  # (T,)
...
class Lit4dVarNet_CROSCIM_inference(Lit4dVarNet_CROSCIM):
    ...
    def __init__(self, *args, test_metrics=None, pre_metric_fn=None, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        res_key = f"patch_x{res}"
        last = self.len_daw[res] - self.frcst_lead
        ...
        for i, var in enumerate(self.tgt_vars):
            if hasattr(batch, var):
                tgt_frcst[var] = getattr(batch, var)[:,-self.frcst_lead:,:,:]
            else:
                raise ValueError(f"Batch missing target variable '{var}'")
        ...
        if dataloader_idx > 0:
            ...
            coarse = {
               k: v.isel(time=np.arange((self.len_daw[coarser_res]-self.frcst_lead)-last,
                                        (self.len_daw[coarser_res]-self.frcst_lead)))
               for k, v in coarse.items()
            }
            sbatch = self.cut_batch(batch)
            coarse = self.convert_xr_to_batch(coarse, sbatch)       
        ...
        for i, var in enumerate(self.tgt_vars):
            ...
            tgt_norm[var] = getattr(batch, var)[:,:-self.frcst_lead,:,:]
            if dataloader_idx > 0:
                tgt_norm[var] += itrp_coarse[pred_var_name]
            if res == self.multires[-1]:
                for k, v in tgt_norm.items():
                    print(f"tgt_norm[{k}] shape before adding frcst: {v.shape}")
                tgt_norm[var] = torch.cat([
                    tgt_norm[var],
                    tgt_frcst[var]
                ], dim=1)
                for k, v in tgt_norm.items():
                    print(f"tgt_norm[{k}] shape after adding frcst: {v.shape}")

        #Forecast pour la dernière résolution
        #####################################
        if res == self.multires[-1]:
            # Create input field
            input_tensors = []
            ...
            for var_name in self.tgt_vars:
                ...
                if '_' in var_name:
                    ...
                    pred_var_name = f'pred_{var_name}'
                input_tensors.append(out_norm[pred_var_name])  # Add the reconstructed variable as input for forecasting
            ...
            for var_name in self.tgt_vars:
                ...
                out_norm[pred_var_name] = torch.cat([
                    out_norm[pred_var_name],
                    out_frcst[pred_var_name]
                                    ], dim=1)
                out_norm[pred_var_name] = torch.where(batch.land_mask==1., np.nan, out_norm[pred_var_name])
        ...
        for k, v in out_norm.items():
            print(f"out_norm[{k}] shape: {v.shape}")

        for k, v in tgt_norm.items():
            print(f"tgt_norm[{k}] shape: {v.shape}")
        ...
        self.test_times[res_key].append(torch.squeeze(batch.time[:,:-self.frcst_lead], dim=1))
        ...
        if self.is_last_batch(batch_idx, dataloader_idx):
            ...
            if dataloader_idx == (len(self.multires)-1):
                ...
                write_netcdf = True
            if res != self.multires[-1]:
                idx_rec = idx_rec[:-self.frcst_lead]
    ...
    def aggregate_batches(self, idx_rec, 
        ...
        for idx_daw in torch.unique(daws):
            ...
            if metrics and res == self.multires[-1]:
                metric_data = test_data_unnorm.pipe(self.pre_metric_fn),
                metrics_val = pd.Series({
                    metric_n: metric_fn(metric_data)
                    for metric_n, metric_fn in self.metrics.items()
                })
                print(metrics_val.to_frame(name="Metrics").to_markdown())
            # save NetCDFs
            ...
            if self.logger and write_netcdf:
                 ...
                 if metrics and res == self.multires[-1]:
                     self.logger.log_metrics(metrics_val.to_dict())
            # stack each daw
