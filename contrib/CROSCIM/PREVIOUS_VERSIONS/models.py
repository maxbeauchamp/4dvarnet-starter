...
class Lit4dVarNet_CROSCIM(Lit4dVarNet):
        ...
        for res, samples in self.test_viz.items():
            ...
            for batch_idx, (input_dict, tgt_dict, out_dict) in enumerate(samples):
                ...
                for var in vars_list:
                    ...
                    last_slices = torch.arange(max(0, T-3), T)
    ...
    def configure_optimizers(self):
        if self.opt_fn is not None:
            ...
            opt = torch.optim.Adam(params, lr=1e-4, weight_decay=1e-5)
            return {
               "optimizer": opt,
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_res_min = batch[f"patch_x{res_min}"]

        total_valid = 0
        total_pixels = 0

        for var_name in self.tgt_vars:
            target = getattr(batch_res_min, var_name)
            total_valid += target.isfinite().sum().item()
            total_pixels += target.numel()

        valid_ratio = total_valid / total_pixels if total_pixels > 0 else 0.0

        # ----------- DEBUG PRINT -----------
        if self.trainer.is_global_zero:
            print(
                f"[Batch {batch_idx}] "
                f"Valid pixels: {total_valid} / {total_pixels} "
                f"({valid_ratio:.4f})"
            )

        if valid_ratio < 0.02:
            if self.trainer.is_global_zero:
                print(f"[Batch {batch_idx}] Skipping batch (valid_ratio={valid_ratio:.4f})")

            # Return safe zero loss
    ...
    def on_after_backward(self):
        ...
        Logs gradient statistics and replaces NaN/Inf in gradients.
        """
        if not self.hook_backward:
            return

        # Only run on global zero in distributed training
        if not self.trainer.is_global_zero:
            return

        print(f"\n🔎 Checking gradients after backward at step {self.global_step}")
        ...
        for name, param in self.named_parameters():
            ...
            # Replace NaN/Inf with 0 to avoid corrupting parameters
            if torch.isnan(grad).any() or torch.isinf(grad).any():
                print(f"⚠️ NaN/Inf detected in gradient of '{name}', replacing with 0")
                param.grad = torch.nan_to_num(grad, nan=0.0, posinf=0.0, neginf=0.0)
                grad = param.grad

            norm = grad.norm().item()
            print(f"{name} grad norm: {norm:.6e}")
        ...
        if grad_norms:
            ...
            total_norm = np.sqrt(np.sum(np.square(grad_norms)))

            print(f"✅ All gradients finite after replacement")
            print(f"📏 Gradients stats - Count: {len(grad_norms)}, "
                f"Mean: {mean_norm:.6e}, Max: {max_norm:.6e}, Min: {min_norm:.6e}")
            print(f"📐 Total gradient norm: {total_norm:.6e}")
        else:
            print("⚠️ No non-zero gradients found on this step")




    def validation_step(self, batch, batch_idx):
        #breakpoint()  # Debug: inspect validation batch and outputs
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        print("\n🔎 Checking parameters after optimizer step")

        for name, param in self.named_parameters():
            if torch.isnan(param).any():
                print(f"💥 NaN appeared in parameter {name}")
                return
            if torch.isinf(param).any():
                print(f"💥 Inf appeared in parameter {name}")
                return

        print("✅ Parameters still finite")
    ...
    def on_train_epoch_start(self):
        ...
        if strategy == 'progressive':
            ...
            warmup_epochs = int(self.trainer.max_epochs * 0.5)
            ...
            if epoch < warmup_epochs:
                ...
                train_res = self.multires  # All resolutions
            ...
            epoch = self.current_epoch
            total_epochs = self.trainer.max_epochs
            steps_per_res = max(1, total_epochs // 2)
            res_index = epoch // steps_per_res
            train_res = [self.multires[res_index], self.multires[res_index+1]]
    ...
    def multistep(self, batch, phase=""):
        ...
        if strategy == 'progressive':
            ...
            epoch = self.current_epoch
            n_res = len(self.multires)
            # Train progressively for first 50% of epochs, then all together
            warmup_epochs = int(self.trainer.max_epochs * 0.5)
            ...
            steps_per_res = max(1, total_epochs // 2)
            res_index = epoch // steps_per_res
            train_resolutions = [self.multires[res_index], self.multires[res_index+1]]
            ##################################
        ...
        for i, res in enumerate(self.multires):
            ...
            if i == 0:
                ...
                if strategy == 'simultaneous' or (strategy == 'hybrid' and self.current_epoch >= warmup_epochs):
...
# ...existing code...
    ...
    def base_step(self, batch, res, phase=""):
        ...
        for i, var_name in enumerate(self.tgt_vars):
            ...
            if self.global_step % 1 == 0 and self.trainer.is_global_zero:
            ...
            # If neither interpolated nor observed pixels exist we skip loss
            # computing NaN-based losses; set them to zero instead of skipping
            if n_mask/n_total < 0.02 and n_mask2/n_total < 0.02:
                if self.global_step % 50 == 0 and self.trainer.is_global_zero:
                    print(f"  [Warning] no valid pixels for {var_name}, forcing zero losses")
                # log percentages (both zero) for bookkeeping
                ...
                # create zero tensors to fall through to the usual addition logic
        ...
        #############################################################
            # accumulate observation counts for summary
            if 'sum_n_mask2' not in locals():
                sum_n_mask2 = 0
                sum_n_total = 0
            sum_n_mask2 += n_mask2
            sum_n_total += n_total
        ...
        # --- summary printout at end of base_step ---
        if self.trainer.is_global_zero:
            pct_obs = 100.0 * sum_n_mask2 / sum_n_total if sum_n_total > 0 else 0.0
            print(f"[Step {self.global_step}] Res {res} {phase}: observed {pct_obs:.2f}% of pixels, total_loss {total_loss.item():.6f}")
        ###############################################################
    ...
    def base_step(self, batch, res, phase=""):
        ...
        if self.trainer.is_global_zero:
            print("\n================= DEBUG BEFORE FORWARD =================")
            print(f"[Step {self.global_step}] Res {res} - Phase: {phase}")
            ...
            x_valid = x[~torch.isnan(x)]
            if x_valid.numel() > 0:
                print("Input min:", x_valid.min().item())
                print("Input max:", x_valid.max().item())
                print("Input mean:", x_valid.mean().item())
                print("Input std:", x_valid.std().item())
            else:
                print("Input has NO valid values.")

            max_abs = torch.nan_to_num(x.abs(), nan=0.0).max().item()
            print("Max absolute value:", max_abs)
            if max_abs > 1e4:
                print("⚠️ WARNING: Large magnitude detected (>1e4)")

            # Check model parameters
            model = self.solver.solvers[f"solver_x{res}"]
            print("Checking model parameters for NaN/Inf...")
        ...
        # ==========================================================
        try:
            out_tensor = self(batch=sbatch, res=res)
        except Exception as e:
            print("❌ Forward crashed:", e)
            raise

        # Output debug BEFORE split
        if self.trainer.is_global_zero:
            print("Output contains NaN:", torch.isnan(out_tensor).any().item())
            print("Output contains Inf:", torch.isinf(out_tensor).any().item())

            out_valid = out_tensor[~torch.isnan(out_tensor)]
            if out_valid.numel() > 0:
                print("Output min:", out_valid.min().item())
                print("Output max:", out_valid.max().item())
                print("Output mean:", out_valid.mean().item())
            else:
                print("Output has NO valid values.")

        out = self.split_tensor_to_dict(out_tensor)
        ...
        for var_name in self.tgt_vars:
            ...
            n_target_valid = target.isfinite().sum().item()
            n_mask = mask.sum().item()
            n_mask2 = mask2.sum().item()
            n_total = target.numel()

            pct_valid = 100.0 * n_target_valid / n_total if n_total > 0 else 0.0
            pct_mask = 100.0 * n_mask / n_total if n_total > 0 else 0.0
            pct_mask2 = 100.0 * n_mask2 / n_total if n_total > 0 else 0.0

            if self.trainer.is_global_zero:
                print(f"\n[Step {self.global_step}] Res {res} - {phase} - Variable: {var_name}")
                print(f"  Mask interp: {pct_mask:.2f}%")
                print(f"  Mask obs:    {pct_mask2:.2f}%")
                print(f"  Target valid: {pct_valid:.2f}%")
                print(f"  Pred contains NaN: {torch.isnan(pred).any().item()}")
        ...
        if self.trainer.is_global_zero:
            pct_obs = 100.0 * sum_n_mask2 / sum_n_total if sum_n_total > 0 else 0.0
            print(f"[Step {self.global_step}] Res {res} {phase}: "
                f"observed {pct_obs:.2f}% - total_loss {total_loss.item():.6f}")
