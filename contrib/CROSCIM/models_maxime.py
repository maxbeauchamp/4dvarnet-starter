...
class Lit4dVarNet_CROSCIM(Lit4dVarNet):
        ...
        for key, item in batch.items():
            ...
            for var in item_dict:
                ...
                if isinstance(data, torch.Tensor) and data.ndim == 4 and data.shape[1] > 1:
                    ...
                    if self.frcst_lead is not None and self.frcst_lead > 0 and var not in self.tgt_vars:
    ...
    def multistep(self, batch, phase=""):
        ...
        print(f"[MULTISTEP] strategy={strategy}")
        ...
        if strategy == 'progressive':
            ...
            print(f"[HYBRID] warmup_epochs={warmup_epochs}")
            ...
            if epoch < warmup_epochs:
                ...
                print(f"[HYBRID] progressive phase")
                print(f"[HYBRID] steps_per_res={steps_per_res}")
                print(f"[HYBRID] res_index={res_index}")

            else:
                train_resolutions = self.multires
                print("[HYBRID] simultaneous phase")

        else:
            train_resolutions = self.multires
            print("[SIMULTANEOUS] training all resolutions")
            ...
            if i == 0:
                ...
                if should_train:
                    ...
                    if loss > 0: 
                        ...
                        mask = torch.isfinite( getattr(batch_res, 'asip_sic'))
                        print("valid pixels:", mask.sum().item(), "/", mask.numel())

                        err = (out[f"patch_x{res}"]['pred_sic'] -  getattr(batch_res, 'asip_sic'))[mask]
                ...
                if strategy == 'simultaneous':
                    print("[GRAD FLOW] no detach between resolutions")
                    out_coarse_for_interp = out[f"patch_x{coarser_res}"]
