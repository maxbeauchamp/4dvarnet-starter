...
#ckpt_path=/homes/m19beauc/4dvarnet-starter/ckpt/sst_dmi_baltic_wcoarse_wgeo_new.ckpt
ckpt_path=/homes/m19beauc/4dvarnet-starter/ckpt/sst_dmi_baltic_wcoarse_wgeo_old.ckpt
#ckpt_path=/homes/m19beauc/4dvarnet-starter/ckpt/sst_dmi_baltic_dm1_wcoarse_wgeo_new.ckpt
...
for month in {1..12} ; do
  ...
  cp -rf config/xp/sed_dmi_sst_test_wcoarse_wgeo.yaml config/xp/dmi_sst_test.yaml
  sed -i -e 's|__START__|'${start}'|g' config/xp/dmi_sst_test.yaml
  sed -i -e 's|__STOP__|'${stop}'|g' config/xp/dmi_sst_test.yaml
  sed -i -e 's|__CKPT_PATH__|'${ckpt_path}'|g' config/xp/dmi_sst_test.yaml
  sed -i -e 's|__DOMAIN__|'${dm}'|g' config/xp/dmi_sst_test.yaml
  #CUDA_VISIBLE_DEVICES=5 HYDRA_FULL_ERROR=1 python main.py xp=dmi_sst_test
  HYDRA_FULL_ERROR=1 CUDA_VISIBLE_DEVICES=4 python main.py xp=dmi_sst_test
  ...
  rm -rf config/xp/dmi_sst_test.yaml
done
