...
class ConsistencyTrainingDynamicalSystems:
    ...
    def __call__(
        ...
        **kwargs : Any
        ...
        intermediate_from_current_x = model_dynamical_systems_forward_wrapper(
                student_model,
                current_noisy_x,
                y,
                current_times,
        ...
        with torch.no_grad():
            ...
            if next_times<=physical_steps[1]:
                current_noisy_x2 = x[:,[0],:,:] + pad_dims_like(compute_sigma(current_times,
                                                                            physical_lag,
                                                                            self.sigma_min,
                                                                            self.sigma_max), x[:,[0],:,:]) * noise
            else:
                idx = torch.searchsorted(physical_steps, current_times).item()
                a = (physical_steps[idx]-next_times)/physical_lag
                current_noisy_x2 = a*x[:,[idx-1],:,:] + (1-a)*x[:,[idx],:,:]
